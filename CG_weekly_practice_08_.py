...
def render(ang):
    ...
    # xang = np.radians(ang)
    # yang = np.radians(30)
    zang0 = np.radians(gAlpha)
    xang = np.radians(gBeta)
    zang1 = np.radians(gGamma)
    M = np.identity(4)
    # Rx = np.array([[1,0,0],
    ...
    Rz0 = np.array([[np.cos(zang0), -np.sin(zang0), 0],
                   [np.sin(zang0), np.cos(zang0), 0],
                   [0,0,1]])
    Rx = np.array([[1, 0, 0],
                   [0, np.cos(xang), -np.sin(xang)],
                   [0, np.sin(xang), np.cos(xang)]])
    Rz1 = np.array([[np.cos(zang1), -np.sin(zang1), 0],
                   [np.sin(zang1), np.cos(zang1), 0],
                   [0, 0, 1]])
    M[:3,:3] = Rz0 @ Rx @ Rz1
    glMultMatrixf(M.T)
...
def main():
    ...
    if not glfw.init():
        return
    window = glfw.create_window(480, 480,'CG_weekly_practice_08_', None,None)
    if not window:
        glfw.terminate()
        return
