...
def shade(camera_viewpoint, lights, l_index, surfaces, s_index, normal_plain, inter_point):
    if is_shadow(light=lights[l_index], surfaces=surfaces, s_index=s_index, inter_point=inter_point):
        ...
        sh = phong(camera_viewpoint=camera_viewpoint, light=lights[l_index], surface=surfaces[s_index], inter_point=inter_point,
                   normal_plain=normal_plain)
    elif surfaces[s_index].shader.type == 'Lambertian':
        sh = lambertian(light=lights[l_index], surface=surfaces[s_index], inter_point=inter_point,
                        normal_plain=normal_plain)
    return sh
        ...
        for y in range(imgSize[1]):
            ...
            if len(ray_result) != 0:
                ...
                for li, light in enumerate(lights):
                    ...
                    sh += shade(camera_viewpoint=cam.viewPoint, lights=lights, l_index=li, surfaces=surfaces, s_index=si, inter_point=ip,
                               normal_plain=nor_p)
