...
def is_shadow(light, surfaces, s_index, inter_point):
    ...
    for si, s in enumerate(surfaces):
        if si == s_index:
            continue
        # multip.append(Process(target=ray_intersection, args=(inter_point, )))
        ret = ray_intersection(camera_viewpoint=inter_point, view_dir=light.position-inter_point, surface=s)
        if ret[0]:
            return True
    return False
